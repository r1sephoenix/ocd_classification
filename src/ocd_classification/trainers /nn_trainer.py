...
def _build_model(input_shape: Tuple[int, ...], cfg: Mapping[str, Any]) -> nn.Module:
    ...
    try:
        ...
        raise ValueError(f"Unknown model_type '{model_key}'. Choose from {list(cls_map)}")
...
def train_nn(
    ...
    *,
    ...
    group_by_subject = cfg.get("group_by_subject", True) and subject_ids_train is not None
    ...
    if group_by_subject:
        ...
        dl_train = DataLoader(ds_train, batch_sampler=sampler, pin_memory=(device.type == "cuda"))
    else:
        dl_train = DataLoader(ds_train, batch_size=bs, shuffle=True, pin_memory=(device.type == "cuda"))

    if X_val is not None and y_val is not None:
        ds_val = TensorDataset(torch.tensor(X_val, dtype=torch.float32),
                               torch.tensor(y_val, dtype=torch.float32))
        dl_val = DataLoader(ds_val, batch_size=bs, shuffle=False, pin_memory=(device.type == "cuda"))
    else:
        dl_val = None
    ...
    for epoch in range(epochs):
        train_loss, train_acc = _run_epoch(model, dl_train, criterion, optimizer, device)
...
def load_model(path: str | Path, input_shape: Tuple[int, ...], cfg: Mapping[str, Any] | None = None) -> nn.Module:
    cfg = dict(cfg or {})
